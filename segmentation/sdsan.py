...
class DeformSparseStripConvolution(nn.Module):
...
class SDSA(nn.Module):
    def __init__(self, embed_chs,kernel_size=5,padding=5,expand=11):
        super().__init__()
        self.dssc = DeformSparseStripConvolution(embed_chs,kernel_size,padding,expand)
...
class Attention(nn.Module):
    def __init__(self, d_model,kernel_size,padding,expand):
        ...
        self.spatial_gating_unit = SDSA(d_model,kernel_size,padding,expand)
...
class SDSAN(BaseModule):
    def __init__(self,
                 ...
                 init_cfg=None,
                 norm_cfg=dict(type='SyncBN', requires_grad=True)):
        super(SDSAN, self).__init__(init_cfg=init_cfg)
        ...
        for i in range(num_stages):
            ...
            block = nn.ModuleList([Layer(dim=embed_dims[i],
